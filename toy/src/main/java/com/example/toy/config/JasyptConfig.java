...
@Component
public class JasyptConfig implements AuthenticationProvider {
    @Autowired
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        Employee employee = userService.getUserById(email);

        if (employee != null && passwordEncoder.matches(password, employee.getEMPLOYEE_PW())) { // 일치하는 user 정보가 있는지 확인
            List<GrantedAuthority> roles = new ArrayList<>();
            roles.add(new SimpleGrantedAuthority("USER")); // 권한 부여

            token = new UsernamePasswordAuthenticationToken(employee.getEMPLOYEE_NO(), null, roles);
